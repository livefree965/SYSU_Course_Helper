...
class DataHandler:
    ...
    def __init__(self, username, password):
        ...
        self.__logData = {
            ...
            "password": password,
            "captcha": None,
            "lt": log_page_soup.find_all('input')[3]['value'],
            "execution": log_page_soup.find_all('input')[4]['value'],
            "_eventId": "submit",
            "submit": "登陆"
        }
    ...
    def Log(self):
        ...
        r = self.__main_session.get(url, stream=True)
        with open('veryfy.jpg', 'wb') as fd:
            fd.write(r.content)
        print(">>>请输入验证码(图片在程序目录下)")
        captcha = input()
        self.__logData['captcha'] = captcha
...
def execute():
    ...
    if user_input == 6:
        ...
        class_input = input()
        if class_input == "help":
            print(">>>第一个数字：1.专选 2.专必 3.公选 4.公必")
            ...
            if len(input_set) == 4:
                COURSE_HELPER.PrintAllCourse(int(input_set[0]) - 1, int(input_set[1]), int(input_set[2]) + 7,
                                             int(input_set[3]))
